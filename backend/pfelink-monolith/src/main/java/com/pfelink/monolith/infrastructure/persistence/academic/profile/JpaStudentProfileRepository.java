...
public class JpaStudentProfileRepository implements IStudentProfileRepository {
    ...
    @Override public StudentProfile save(StudentProfile sp) { return springRepo.save(sp); }
    @Override public Optional<StudentProfile> findById(UUID id) { return springRepo.findById(id); }
    @Override public boolean existsByUserId(UUID uid) { return springRepo.existsByUserId(uid); }
    @Override public Optional<StudentProfile> findByUserId(UUID uid) { return springRepo.findByUserId(uid); }
    @Override public List<StudentProfile> findAllPending() { return springRepo.findByStatus(StudentStatus.PENDING); }
}
