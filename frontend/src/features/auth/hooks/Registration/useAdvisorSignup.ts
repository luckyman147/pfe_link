...
const schema = z.object({
  ...
  facultyDomainEmail: z.string().email('Invalid faculty email').optional().or(z.literal('')),
}).refine(
  (d) => Boolean(d.facultyId) !== Boolean(d.facultyDomainEmail),
  { message: 'Pick a university/faculty or enter its email', path: ['facultyId'] }
);
...
export const useAdvisorSignup = () => {
  ...
  const onSubmit = async (data: AdvisorFormData) => {
    try {
      await registerAdvisor({
        ...
        facultyDomainEmail: data.facultyDomainEmail || undefined,
      });
  ...
  return { form, faculties, cinUrl, draftId, handleCinChange, onSubmit };
};
